...
contract(
    ...
    (
        ...
        fillReservesFn,
        createScheduleFn,
        createSingleCapsuleFn,
        createMultiCapsuleFn
    ) => {
        let capsuleInstance,
            tokenInstance1,
            tokenInstance2,
            treasurer_role,
            beforeEachSnapshot;
        const [treasurer, no_role_account, sender, recipient, account] =
            accounts;
        ...
        const verifyVestedBalance = async (capsuleId, amount) => {
            const vestedBalance = await capsuleInstance.vestedBalanceOf(
                capsuleId
            );
        ...
        before(async () => {
            capsuleInstance = await deployVault();
            tokenInstance1 = await deployERC20();
            tokenInstance2 = await deployERC20();
        ...
        describe("Creating Capsules", async () => {
            ...
            it("should fail when schedule ID is invalid", async () => {
                ...
                await expectRevert(
                    createCapsule(capsule),
                    "VestingVault: Invalid scheduleId"
                );
            });
            ...
            it("should fail when contract's token balance < schedule amount", async () => {
                ...
                await expectRevert(
                    createCapsule(capsule),
                    "VestingVault: Schedule has insufficient token reserves for new capsule."
                );
            });
